    ...
    try {
      ...
      if (iconFile && storage) {
        try {
          ...
          console.warn("Icon upload failed, saving without image asset:", imgErr);
        }
      }

      // Exact data mapping to strictly match your "categories" rule expectations
      const data = {
        name: form.name.trim(),
        menu: form.menu || "",
    ...
    <div className="fixed inset-0 z-50 flex justify-end items-stretch bg-black/60 backdrop-blur-sm">
      <div className="bg-[#1a1a1a] w-full max-w-lg h-full overflow-y-auto shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {/* Select Menu Dropdown Integration */}
          <CustomSelect
            label="Select Menu"
            value={form.menu}
            onChange={(val) => setForm((p) => ({ ...p, menu: val }))}
            options={MENU_OPTIONS}
            placeholder="Select menu"
          />

          {/* Descriptive Info Area */}
          <div>
