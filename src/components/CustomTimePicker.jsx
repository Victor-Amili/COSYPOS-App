...
const ALL_HOURS = Array.from({ length: 13 }, (_, i) => `${String(i + 9).padStart(2, "0")}:00`);
    ...
    <div className="relative" ref={ref}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={value ? "text-white" : "text-white/30"}>{value || "Select time"}</span>
        </div>
            ...
            {ALL_HOURS.map((hour) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  disabled={isDisabled}
                  onClick={() => { if (!isDisabled) { onChange(hour); setOpen(false); } }}
                  className={`py-2 rounded-lg text-xs font-medium transition relative
                    ${isSelected ? "bg-brand text-gray-800" :
                      isDisabled ? "bg-white/5 text-white/20 cursor-not-allowed" :
                      "text-white hover:bg-white/10"}`}
                >
                  {hour}
                  {isDisabled && !isSelected && (
                    <span className="absolute top-0.5 right-0.5 w-1.5 h-1.5 rounded-full bg-red-400/60" />
                  )}
                </button>
              );
            })}
